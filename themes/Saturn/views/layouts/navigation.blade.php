<nav class="bg-secondary-50 dark:bg-zinc-800/60 dark:border-0 border-b-2 border-secondary-200">
    <div class="max-w-[1650px] h-[100px] mx-auto block md:flex items-center gap-x-10 px-5">
        ...
        <div class="w-full justify-between gap-x-10 md:px-5 md:flex hidden" id="mobile-menu">
            @auth
                <a href="{{ route('clients.home') }}"
                    class="md:px-2 py-3 flex items-center gap-x-1 text-white hover:text-secondary-800 duration-300">
                    {{ __('Customer Area') }}
                </a>
            @endauth
            @if(Auth::check() && Auth::user()->has('ADMINISTRATOR'))
                <button type="button" aria-expanded="true" data-dropdown-placement="bottom-start" aria-haspopup="true"
                    ...
                    class="relative md:px-2 py-3 flex items-center gap-x-1 text-white hover:text-secondary-800 duration-300">
                    {{ __('Products') }} <i class="ri-arrow-down-s-line"></i>
            ...
            <div class="ml-auto flex items-center gap-x-1 justify-center md:pb-0 pb-4">
                ...
                @else
                    <a href="{{ route('login') }}" class="button button-primary md:flex-none flex-1 bg-blue-600 hover:bg-blue-700">
                        {{ __('Log In') }}
                    </a>
                @endauth
